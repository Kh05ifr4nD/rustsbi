...
pub fn run(arg: &TestArg) -> Option<ExitStatus> {
    ...
    if arg.pack {
        ...
        match pack_image(&current_dir, &target_dir) {
            Ok(status) => {
                info!(
                    ...
                    target_dir.join(format!("{TEST_KERNEL_NAME}.itb")).display()
                );
                return Some(status);
            }
            Err(err_msg) => {
        ...
        info!(
            ...
            target_dir.join(format!("{TEST_KERNEL_NAME}.bin")).display()
        );
    }
...
fn convert_to_binary(target_dir: &Path) -> Option<ExitStatus> {
    ...
    let bin_path = target_dir.join(format!("{TEST_KERNEL_NAME}.bin"));
...
fn pack_image(current_dir: &Path, target_dir: &Path) -> Result<ExitStatus, String> {
    ...
    let its_source = current_dir
        ...
        .join("test-kernel")
        .join("scripts")
        .join(format!("{TEST_KERNEL_NAME}.its"));

    let its_dest = target_dir.join(format!("{TEST_KERNEL_NAME}.its"));
    ...
    let status = Command::new("mkimage")
        .args([
            "-f",
            &format!("{TEST_KERNEL_NAME}.its"),
            &format!("{TEST_KERNEL_NAME}.itb"),
        ])
        .status()
        .map_err(|e| format!("Failed to execute mkimage command: {e}"))?;

    // Clean up
    fs::remove_file(format!("{TEST_KERNEL_NAME}.its"))
